...
object UserPreferences {
    ...
    fun getRfedNodeIdentityHash(context: Context): String =
        normalizedHex(prefs(context).getString(PREF_KEY_RFED_NODE_IDENTITY_HASH, ""))

    /** Runtime RFed identity hash. Uses the hidden fallback only when Settings is blank. */
    ...
    fun setRfedNodeIdentityHash(context: Context, hex: String) {
        prefs(context).edit()
            .putString(PREF_KEY_RFED_NODE_IDENTITY_HASH, normalizedHex(hex))
            .apply()
    }

    fun getRfedNotifyHash(context: Context): String =
        normalizedHex(prefs(context).getString(PREF_KEY_RFED_NOTIFY_HASH, ""))

    fun setRfedNotifyHash(context: Context, hex: String) {
        prefs(context).edit()
            .putString(PREF_KEY_RFED_NOTIFY_HASH, normalizedHex(hex))
            .apply()
    }

    fun getRfedLxmfPropOverride(context: Context): String =
        normalizedHex(prefs(context).getString(PREF_KEY_RFED_LXMF_PROP_OVERRIDE, ""))

    fun setRfedLxmfPropOverride(context: Context, hex: String) {
        prefs(context).edit()
            .putString(PREF_KEY_RFED_LXMF_PROP_OVERRIDE, normalizedHex(hex))
            .apply()
    }
    ...
    fun getLxmfPropagationHash(context: Context): String =
        normalizedHex(prefs(context).getString(PREF_KEY_LXMF_PROPAGATION_HASH, ""))

    fun setLxmfPropagationHash(context: Context, hex: String) {
        prefs(context).edit()
            .putString(PREF_KEY_LXMF_PROPAGATION_HASH, normalizedHex(hex))
            .apply()
    }
